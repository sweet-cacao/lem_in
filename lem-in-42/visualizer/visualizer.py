...
try:
    root = Tk()
    root.title("LEM-IN")
    icon = '/Users/gstarvin/lem_in/lem-in-42/visualizer/imges/Tatice-Cristal-Intense-Apple-multicolor.icns`'
...
try:
    ...
    for i in range(len(obj)):
        x = obj[i].x*multiple
        y = obj[i].y*multiple
        canvas.create_oval(x-size_r, y-size_r, x+size_r, y+size_r, fill='#00ffff')
    ...
    img = PIL.Image.open('./imges/black-yel.jpg')
    maxsize = (20, 20)
...
canvas.create_text(725,20,text="LEM-IN",
          font="Verdana 20 italic bold",justify=CENTER,fill="#00ffff")

canvas.create_text(1410,40,text=" - start",
          font="Verdana 15",justify=CENTER,fill="#00ffff")
